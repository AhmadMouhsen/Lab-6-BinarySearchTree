...
public class BinarySearchTree<E> {
    ...
    /*
     * private boolean find(Node node, E item) { if (node == null) { return false;
     * // item not found } else if (item.equals(node.data)) { return true; } else if
     * (item.compareTo(node.data) < 0) { return find(node.left, item); } else {
...
//    /*
//     * public boolean remove(E item) { if (root == null) { return false; // empty
//     * tree } else if (item.equals(root.data)) { Node auxRoot = new Node(null);
//     * auxRoot.left = root; boolean result = remove(root, auxRoot, item); root =
//     * auxRoot.left; return result; } else { return remove(root, null, item); } }
//     */
//    /*
//     * private boolean remove(Node node, Node parent, E item) { if (node == null) {
//     * return false; // item not found } else if (item.equals(node.data)) { if
//     * (node.left == null || node.right == null) { // case 1: node has at most one
//     * child Node child = node.left != null ? node.left : node.right; if (node ==
//     * parent.left) { parent.left = child; } else { parent.right = child; } } else {
//     * // case 2: node has two children Node minNode = findMin(node.right);
//     * node.data = minNode.data; remove(node.right, node, minNode.data); } return
//     * true; } else if (item.compareTo(node.data) < 0) { return remove(node.left,
//     * node, item); } else { return remove(node.right, node, item); } }
//     */
